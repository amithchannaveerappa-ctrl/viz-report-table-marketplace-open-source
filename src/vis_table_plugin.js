...
class VisPluginTableModel {
  ...
  constructor(lookerData, queryResponse, config) {
    ...
    this.pivot_fields = [];
    this.pivot_values =
      typeof queryResponse.pivots !== 'undefined' ? queryResponse.pivots : [];
    this.variances = [];
    this.column_series = [];
    ...
    this.hasTotals =
      typeof queryResponse.totals_data !== 'undefined' ? true : false;
    this.calculateOthers =
      typeof queryResponse.truncated !== 'undefined'
        ? queryResponse.truncated && config.calculateOthers
        : false;
    this.hasSubtotals =
      typeof queryResponse.subtotals_data !== 'undefined' ? true : false;
    this.hasRowTotals = queryResponse.has_row_totals || false;
    this.hasPivots = typeof queryResponse.pivots !== 'undefined' ? true : false;
    this.hasSupers =
      typeof queryResponse.fields.supermeasure_like !== 'undefined'
        ? Boolean(queryResponse.fields.supermeasure_like.length)
        : false;
    ...
    if (this.useIndexColumn) {
      this.addIndexColumn(queryResponse);
    }
    ...
    this.measures.forEach((measure, i) => {
      ...
      if (measure.can_pivot) {
        ...
        this.pivot_fields.forEach((pivot_field, p) => {
          if (
            this.pivot_fields.length === 1 ||
            p === 1 ||
            this.config.colSubtotals
          ) {
            var option = {};
      ...
      this.measures.forEach((comparisonMeasure, j) => {
        var includeMeasure =
          measure.can_pivot === comparisonMeasure.can_pivot ||
    ...
    queryResponse.fields.dimension_like.forEach(dimension => {
      ...
      this.headers.forEach(header => {
        switch (header.type) {
          case 'pivot0':
          case 'pivot1':
            var pivotField = new ModelPivot({
              vis: this,
              queryResponseField: header.modelField,
            });
            var headerCell = new HeaderCell({
              column: column,
              type: header.type,
              modelField: pivotField,
            });
    ...
    if (this.hasPivots) {
      this.pivot_values.forEach(pivot_value => {
        ...
        this.measures.forEach((measure, m) => {
          ...
          var include_measure =
            !isRowTotal || (isRowTotal && !measure.is_table_calculation);

          if (include_measure) {
            var column = new Column(
              [pivot_value.key, measure.name].join('.'),
              this,
              measure
            );
              ...
              switch (header.type) {
                ...
                case 'pivot1':
                  ...
                  if (
                    ...
                    header.type === 'pivot' + (this.pivot_fields.length - 1)
                  ) {
                    label = 'Row Total';
                  }
                  column.levels.push(
                  ...
                  level_sort_values.push(
                    pivot_value.sort_values[header.modelField.name]
                  );
                  if (column.pivoted) {
                    tempSort.push({
    ...
    Object.keys(this.config).forEach(option => {
      if (option.startsWith('comparison')) {
        ...
        var comparison_options = [
          ...this.measures.map(measure => measure.name),
          ...this.pivot_fields.map(pivot_field => pivot_field.name),
        ];
        ...
        if (baseline_in_measures && comparison_available) {
          if (
            this.pivot_fields
              .map(pivot_field => pivot_field.name)
              .includes(this.config[option])
          ) {
            var type = 'by_pivot';
          } else {
            var type =
...
*/
  ...
  checkSubtotalsData(queryResponse) {
    if (
      ...
      queryResponse.subtotals_data[this.addSubtotalDepth].forEach(
        lookerSubtotal => {
          var visSubtotal = new Row('subtotal');

          visSubtotal['$$$__grouping__$$$'] =
            lookerSubtotal['$$$__grouping__$$$'];
          var groups = ['Subtotal'];
          ...
          this.columns.forEach(column => {
            ...
            if (typeof cell !== 'undefined') {
              ...
              if (typeof column.modelField.style !== 'undefined') {
                cell.cell_style = cell.cell_style.concat(
                  column.modelField.style
                );
              }
              if (cell.value === null) {
                cell.rendered = '';
              ...
              var reportInSetting =
                this.config['reportIn|' + column.modelField.name];
              if (
                typeof reportInSetting !== 'undefined' &&
                reportInSetting !== '1'
              ) {
                var unit =
    ...
    lookerData.forEach((lookerRow, i) => {
      ...
      if (this.useIndexColumn) {
        ...
        row.data['$$$_index_$$$'] = new DataCell({
          ...
          align: this.dimensions[this.dimensions.length - 1].is_numeric
            ? 'right'
            : 'left',
          colid: '$$$_index_$$$',
  ...
  buildTotals(queryResponse) {
    ...
    this.columns.forEach(column => {
      ...
      if (column.modelField.type === 'dimension') {
        if ([this.firstVisibleDimension, '$$$_index_$$$'].includes(column.id)) {
          var rowspan = 1;
          var colspan = this.useIndexColumn
            ? 1
            : this.dimensions.filter(d => !d.hide).length;
        } else {
          var rowspan = -1;
          var colspan = -1;
        }
      ...
      if (column.modelField.type === 'measure') {
        var cell_style = column.modelField.is_numeric
          ? ['total', 'numeric', 'measure']
          : ['total', 'nonNumeric', 'measure'];
        var cellValue =
          column.pivoted || column.isRowTotal
            ? totals_[column.modelField.name][column.pivot_key]
            : totals_[column.id];
        ...
        if (
          typeof cellValue.rendered === 'undefined' &&
          typeof cellValue.html !== 'undefined'
        ) {
          // totals data may include html but not rendered value
        ...
        if (typeof reportInSetting !== 'undefined' && reportInSetting !== '1') {
          ...
          cellValue.value = Math.round(
            cellValue.value / parseInt(reportInSetting)
          );
          cellValue.rendered =
            column.modelField.value_format === ''
    ...
    if (this.useIndexColumn) {
      ...
      totalsRow.data['$$$_index_$$$'].colspan = this.dimensions.filter(
        d => !d.hide
      ).length;
    } else {
    ...
    if (this.calculateOthers) {
      ...
      this.columns.forEach(column => {
        ...
        var othersStyle = column.modelField.is_numeric
          ? ['numeric']
          : ['nonNumeric'];
        ...
        if (column.modelField.type === 'measure') {
          if (
            (othersValue = ['sum', 'count'].includes(
              column.modelField.calculation_type
            ))
          ) {
  ...
  setRowSpans() {
    ...
    for (var l = leaves.length - 1; l >= 0; l--) {
      ...
      for (var t = 0; t < tiers.length; t++) {
        ...
        var neighbour_value =
          l > 0 ? leaves[l - 1].data[tier.name].value : null;
  ...
  addSubTotals() {
    ...
    subTotalGroups.forEach((subTotalGroup, s) => {
      ...
      this.columns.forEach(column => {
        if (column.modelField.type === 'dimension') {
          if (
            [this.firstVisibleDimension, '$$$_index_$$$'].includes(column.id)
          ) {
            var rowspan = 1;
            var colspan = this.useIndexColumn
              ? 1
          ...
          var cell_style = column.modelField.is_numeric
            ? ['total', 'subtotal', 'numeric', 'dimension']
            : ['total', 'subtotal', 'nonNumeric', 'dimension'];
          var cell = new DataCell({
            cell_style: cell_style,
            align: column.modelField.is_numeric ? 'right' : 'left',
          ...
          if (
            column.id === '$$$_index_$$$' ||
            column.id === this.firstVisibleDimension
          ) {
        ...
        if (column.modelField.type == 'measure') {
          var cell_style = column.modelField.is_numeric
            ? ['total', 'subtotal', 'numeric', 'measure']
  ...
  addColumnSubTotals() {
    ...
    pivots.forEach(pivot => {
      this.measures.forEach((measure, m) => {
        if (measure.can_pivot) {
          var subtotalColumn = new Column(
            ['$$$_subtotal_$$$', pivot, measure.name].join('.'),
            this,
            measure
          );
            ...
            for (var i = 0; i < column.levels.length; i++) {
              if (column.levels[i].type.startsWith('pivot')) {
                ...
                if (typeof column.levels[i].pivotData.data !== 'undefined') {
                  columnPivotValue =
                    column.levels[i].pivotData.data[pivotDimension];
                }
                break;
              }
            }
      ...
      this.headers.forEach((header, i) => {
        switch (header.type) {
          case 'pivot0':
            ...
            var sortValueFromColumn =
              subtotalColumn.subtotal_data.columns[0].levels[i].pivotData
                .sort_values[header.modelField.name];
            subtotalColumn.levels.push(
              new HeaderCell({
            ...
            subtotalColumn.sort.push({
              name: header.modelField.name,
              value: sortValueFromColumn,
            });
            break;

          case 'pivot1':
            console.log('line1453');
            subtotalColumn.levels.push(
              new HeaderCell({
                column: subtotalColumn,
                type: header.type,
            ...
            var sortOption = this.sorts.find(
              sort => sort.name === header.modelField.name
            );
            if (
              typeof sortOption === 'undefined' ||
              typeof sortOption.desc === 'undefined'
            ) {
            ...
            if (sortDescending) {
              var subtotalSortValue =
                typeof this.pivot_values[0].sort_values[
          ...
          case 'heading':
            subtotalColumn.levels.push(
              new HeaderCell({
                column: subtotalColumn,
                type: 'heading',
                modelField: subtotalColumn.modelField,
              })
            );
            break;
          ...
          case 'field':
            ...
            subtotalColumn.sort.push({
              name: 'measure_idx',
              value: subtotalColumn.subtotal_data.measure_idx,
            });
            break;
        }
      });
    ...
    this.data.forEach(row => {
      subtotalColumns.forEach(subtotalColumn => {
        var cell_style = subtotalColumn.modelField.is_numeric
          ? ['subtotal', 'numeric', 'measure']
          : ['subtotal', 'nonNumeric', 'measure'];
        var subtotal_value = 0;
        subtotalColumn.subtotal_data.columns.forEach(column => {
          // subtotalColumn.columns i.e. the individual columns that are aggregated into a single subtotal columns
  ...
  createVarianceColumn(colpair) {
    if (
      !this.config.colSubtotals &&
      colpair.variance.baseline.startsWith('$$$_subtotal_$$$')
    ) {
      console.log(
        'Cannot calculate variance of column subtotals if subtotals disabled.'
      );
      return;
    }
    var id = [
      '$$$_variance_$$$',
      colpair.calc,
      colpair.variance.baseline,
      colpair.variance.comparison,
    ].join('|');
      ...
      switch (header.type) {
        ...
        case 'heading':
          var headerCell = new HeaderCell({
            column: column,
            type: 'heading',
            modelField: baseline.modelField,
          });
          column.levels[i] = headerCell;
          break;
        case 'field':
    ...
    if (colpair.variance.reverse) {
      this.calculateVariance(
        baseline.modelField.value_format,
        id,
        colpair.calc,
        comparison,
        baseline
    ...
    Object.keys(this.variances).forEach(v => {
      ...
      if (variance.comparison !== 'no_variance') {
        if (variance.type === 'vs_measure') {
          ...
          if (
            this.pivot_fields.length === 1 ||
            this.pivot_fields[1].name === variance.comparison
          ) {
            ...
            // top pivot value - variance by subtotal
  ...
  compareSortArrays(dataTable) {
    return function (a, b) {
      ...
      for (var i = 0; i < depth; i++) {
        var field =
          a.sort[i] && typeof a.sort[i].name !== 'undefined'
            ? a.sort[i].name
            : '';
        ...
        var a_value =
          a.sort[i] && typeof a.sort[i] !== 'undefined' ? a.sort[i].value : 0;
        var b_value =
          b.sort[i] && typeof b.sort[i] !== 'undefined' ? b.sort[i].value : 0;
  ...
  setColSpans() {
    ...
    for (var l = leaves.length - 1; l >= 0; l--) {
      ...
      for (var t = 0; t < tiers.length; t++) {
        ...
        var neighbour_value =
          l > 0 ? leaves[l - 1].data[tier.type].label : null;
  ...
  transposeRowsIntoColumns() {
    ...
    this.data.forEach(sourceRow => {
      ...
      this.transposed_headers.forEach(header => {
        var cellRef =
          this.useIndexColumn && ['subtotal', 'total'].includes(sourceRow.type)
            ? '$$$_index_$$$'
            : header.modelField.name;
        ...
        var headerCell = new HeaderCell({
          ...
          label:
            sourceCell.rendered === ''
              ? sourceCell.rendered
              : sourceCell.rendered || sourceCell.value,
          align: 'center',
          cell_style: sourceCell.cell_style,
        });
        headerCell.colspan = sourceCell.rowspan;
      ...
      .forEach(column => {
        ...
        column.levels.forEach((level, i) => {
          ...
          switch (level.type) {
            ...
            case 'heading':
            case 'field':
              var style = column.modelField.is_table_calculation
                ? 'calculation'
                : 'measure';
  ...
  getRenderedFromHtml(cellValue) {
    ...
    if (
      typeof cellValue.html !== 'undefined' &&
      !['undefined', ''].includes(cellValue.html)
    ) {
      try {
  ...
  getTableHeaderCells(i) {
    if (!this.transposeTable) {
      ...
      return this.transposed_columns.filter(c => c.levels[i].colspan > 0);
    }
  }
  ...
  getCellToolTip(rowid, colid) {
    ...
    if (row.type === 'total') {
      ...
      tipHTML += [
        '<tr class="',
        rowClass,
        '"><td><span style="float:left"><em>',
        label,
        ':</em></td><td></span><span style="float:left"> ',
        value,
        '</span></td></tr>',
      ].join('');
      ...
      var label = 'SUBTOTAL';
      var rowClass = 'focus';
      var subtotalColumn = this.columns
        .filter(c => !c.hide)
        .filter(c => c.modelField.type === 'dimension')[0];
      var value =
        row.data[subtotalColumn.id].render || row.data[subtotalColumn.id].value;
      tipHTML += [
        '<tr class="',
        rowClass,
        '"><td><span style="float:left"><em>',
    ...
    measureColumns.forEach(column => {
      ...
      if (
        (!column.pivoted && !column.isRowTotal) ||
        column.pivot_key === focusColumn.pivot_key
      ) {
        var label = column.getHeaderCellLabelByType('field');
        var rowClass = column.id === focusColumn.id ? 'focus' : '';
        ...
        tipHTML += [
          ...
          '"><td><span style="float:left"><em>',
          label,
          ':</em></td><td></span><span style="float:right"> ',
          value,
          '</span></td></tr>',
        ].join('');
      }
    ...
    if (typeof reportInSetting !== 'undefined' && reportInSetting !== '1') {
      isReportedIn =
        measureLabel + ' reported in ' + reportInLabels[reportInSetting];
    }
    ...
    if (isReportedIn) {
      tipHTML +=
        '<tr><td colspan=2><span style="color:darkgrey">' +
        isReportedIn +
        '.</span></td></tr>';
    }
    ...
    if (isEstimate) {
      tipHTML +=
        '<tr><td colspan=2><span style="color:red">Estimated figure due to query exceeding row limit.</span></td></tr>';
      tipHTML +=
        '<tr><td colspan=2><span style="color:red">Consider increasing the row limit or using an alternative measure.</span></td></tr>';
    }
